...
app.post("/sucessful", function (req, res) {
  ...
  let book_values = [
    [
      ...
      sess.booking.name,
      sess.booking.RoomID,
      sess.booking.countOfRooms,
      sess.booking.checkInDate,
      sess.booking.checkInTime,
      sess.booking.checkOutDate,
      sess.booking.days,
    ],
  ];
  ...
  let sql =
    "INSERT INTO bookings (ClientID,Name,RoomID,NumberOfRooms,checkInDate,checkInTime,checkOutDate,Days) VALUES ?";
  connection.query(sql, [book_values], function (err, result) {
    if (err) {
      ...
      let room_count = 0;
      let check_avail =
        "SELECT RoomType,Rooms_available FROM rooms WHERE RoomID=?";
      connection.query(
        check_avail,
        [sess.booking.RoomID],
        function (er, rows, fields) {
          if (er) {
            ...
            let update_room_count =
              "UPDATE rooms SET Rooms_available = ? WHERE RoomID= ?; ";
                  ...
                  connection.query(
                    ...
                    function (er, row, field) {
                      if (er) {
                        ...
                        let insert =
                          "INSERT INTO payment (BookingID,Card_Number,Card_Expiration_date,CVV,Owner_name) VALUES ?";
                        bcrypt.hash(
                          card_no,
                          saltRounds,
  ...
  connection.query(check_query, [mail, user], function (er, rows, fields) {
    if (er) {
      ...
      if (rows.length != 0) {
        ...
        let values = [
          [fname, lname, dateOfBirth, gender, mail, phone, user, pass],
        ];

        //console.log("PASSWORD1: ");
        //console.log(pass);
        let sql =
          "INSERT INTO clients (First_Name,Last_Name,DOB,Gender,Email,Contact_Number,username,password) VALUES (?)";
  ...
  connection.query(check_avail, [room], function (er, rows, fields) {
    if (er) {
      ...
      if (rows[0].Rooms_available - countOfRooms <= 0) {
        ...
        sess.book_head =
          "Only " +
          String(rows[0].Rooms_available) +
          " " +
          rows[0].RoomType +
          " available, requirement can't be satisfied. Please Try Again!";
        res.redirect("/book");
      } else {
        //ROOMS ARE AVAILABLE
